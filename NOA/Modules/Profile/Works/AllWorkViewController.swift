...
class AllWorkViewController: UIViewController {
...
extension AllWorkViewController {
    // MARK: - UI Binding
    func setupBindings() {
        // ------------------------------
        //     INPUT
        ...
        Observable.merge([firstLoad, reload])
            .bind(to: self.viewModel.input.fetchAllList)
            .disposed(by: disposeBag)
        
        // 무한 스크롤
        self.collectionView.rx_reachedBottom
            .map { _ in () }
            .bind(to: self.viewModel.input.moreFetchAllList)
            .disposed(by: disposeBag)
...
extension AllWorkViewController: UICollectionViewDelegateFlowLayout {
